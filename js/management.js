...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const insights = [
      `3 user accounts (${Object.keys(USER_STATUS).map(id => D.userById(id).name).join(", ")}) have not logged in for over 30 days and are marked Inactive.`,
      "Milestone master 'Further Review' is currently applied to 34% of active legislation cases — consider reviewing directorate review capacity.",
      "Case Type Supervisor and Directorate Power Admin roles have not been assigned to any user yet — 2 elevated roles are currently unused.",
      "Document Bank template 'Translation Request Form.pdf' was reused in 6 cases this month, the highest of any template.",
    ];
    document.getElementById("mgmtAiInsights").innerHTML = insights.map(t => `<div class="ai-insight-item"><i class="bi bi-stars"></i><div>${t}</div></div>`).join("")
